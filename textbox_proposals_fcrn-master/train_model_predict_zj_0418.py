...
def fcrn_loss_new(y_true, y_pred):
    ...
    for i in range(0, mini_batch_size):
        ...
        print (u_true.size)
        if u_true.size<256: 
            print (type(K.mean(y_true)))        
            return K.mean(y_true)#shared(loss)#K.int_shape(loss)#K.variable(value=np.array(loss), dtype='float32', name=None)
    ...
    return shared(loss)#K.int_shape(loss)#K.variable(value=np.array(loss), dtype='float32', name=None)           
...
def build_model():
    ...
    model.compile(loss = fcrn_loss_new, optimizer = sgd, metrics = ['accuracy'])
...
if __name__ == '__main__':
  model_file = "bb-fcrn-model_weight"
  ...
  if os.path.exists(model_file + ".h5"):
    ...
    print (".........")
    result = nms(boxes,confideres,0.6)
    print ("---------")
